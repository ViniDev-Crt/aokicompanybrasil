...
// ===== COMPONENTE: SEÇÃO DE VÍDEO FIXO =====
const VideoUploadSection = () => {
  return (
    <div
      className="w-full py-16 px-4"
      style={{ background: "linear-gradient(135deg, hsl(var(--secondary)/0.3) 0%, hsl(var(--background)) 100%)" }}
    >
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-foreground">
            Veja o Robô em{" "}
            <span style={{ color: "hsl(var(--primary))" }}>Ação</span>
          </h2>
          <p className="text-muted-foreground mt-2 max-w-xl mx-auto">
            Assista ao nosso Robô Sinalizador Viário funcionando em condições reais de trabalho
          </p>
        </div>
        ...
        <div className="relative bg-card rounded-2xl border shadow-card overflow-hidden">
          {/* Glow decorativo */}
          <div className="absolute inset-0 bg-gradient-to-tr from-primary/10 to-accent/10 rounded-2xl pointer-events-none z-10" />

          <video
            src="/videos/VideoAoki.mp4"
            controls
            autoPlay
            loop
            muted
            playsInline
            className="w-full max-h-[520px] object-contain bg-black rounded-2xl relative z-0"
          />

          {/* Badges de destaque */}
          <div className="absolute top-4 left-4 z-20 flex gap-2">
            <span className="inline-flex items-center gap-1 bg-background/90 backdrop-blur-sm text-primary text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-primary/20">
          ...
          <div className="absolute top-4 right-4 z-20">
            <span className="inline-flex items-center gap-1 bg-background/90 backdrop-blur-sm text-foreground text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg border">
              🤖 Robô Sinalizador
            </span>
          </div>
        </div>
        ...
        <div className="text-center mt-8">
          <p className="text-muted-foreground text-sm mb-4">
            Impressionado? Entre em contato e solicite uma demonstração presencial.
          </p>
          <a
            href="https://wa.me/5518988082235?text=Olá! Gostaria de agendar uma demonstração do Robô Sinalizador."
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-200 hover:translate-y-[-2px] shadow-lg"
          >
              ...
              <div className="flex justify-center gap-2 mt-6">
                {productImages.map((_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-all duration-200 ${index === selectedImage ? 'bg-primary' : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'
                      }`}
                  />
                ))}
              </div>
